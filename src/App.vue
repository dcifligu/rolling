<script setup>
import Circles from './components/Circles.svg.vue';
...
function change_color(){
  ...
  gsap.to("#circle", { x: 320, duration: 0.5, ease: "power2.inOut" });

  // Move text to the left
  gsap.to("#buttontxt", { x: -100, duration: 0.5, ease: "power2.inOut" });

  // Swap text after animation completes
  ...
  // Change button background & text color
...
<template>
  <div id="main_bg" class="bg-[#F6F6F6] bg-noise h-screen w-screen relative z-10 overflow-hidden transition-all duration-300">
    <Circles class="absolute z-20"/>
    <div class="absolute z-30 w-full h-full flex items-center justify-center">
      ...
      <div class="p-8 w-full h-full">
        <div class="flex w-full h-full items-end">
          <div id="button" class="bg-[#1E1E1E] w-fit h-fit p-2 rounded-full cursor-pointer" @click="change_color()">
            <div class="flex gap-6 h-fit w-full items-center justify-between relative">
                ...
                <div class="w-fit pr-6 h-fit flex flex-col items-center justify-center">
                    <span id="buttontxt" class="text-white text-3xl font-thin montserrat tracking-wide uppercase">
                        Learn <br> more about us
                    </span>
                </div>
            </div>
          </div>
